...
namespace
{
volatile uint32_t& Register(uintptr_t base, uint32_t offset)
{
...
{
  return gpio_base_ != 0u && (Register(gpio_base_, REG_EXT_PORTA) & pin_mask_) != 0u;
}
...
void SG200XGPIO::Write(bool value)
{
  ...
  auto& direction = Register(gpio_base_, REG_DDR);
  ...
  auto& data = Register(gpio_base_, REG_DR);
...
ErrorCode SG200XGPIO::SetConfig(Configuration config)
{
  ...
  auto& interrupt_enable = Register(gpio_base_, REG_INTEN);
  auto& interrupt_mask = Register(gpio_base_, REG_INTMASK);
  auto& interrupt_type = Register(gpio_base_, REG_INTTYPE_LEVEL);
  auto& interrupt_polarity = Register(gpio_base_, REG_INT_POLARITY);
  auto& direction = Register(gpio_base_, REG_DDR);

  interrupt_enabled_ = false;
  interrupt_enable &= ~pin_mask_;
  interrupt_mask |= pin_mask_;
  Register(gpio_base_, REG_EOI) = pin_mask_;
  ...
  switch (config.direction)
  {
    ...
    case Direction::OUTPUT_OPEN_DRAIN:
      direction |= pin_mask_;
      interrupt_type &= ~pin_mask_;
      Register(gpio_base_, REG_DR) &= ~pin_mask_;
      break;
...
ErrorCode SG200XGPIO::EnableInterrupt()
{
  ...
  Register(gpio_base_, REG_EOI) = pin_mask_;
  Register(gpio_base_, REG_INTMASK) &= ~pin_mask_;
  Register(gpio_base_, REG_INTEN) |= pin_mask_;
...
void SG200XGPIO::CheckInterrupt(uintptr_t gpio_base)
{
  ...
  const uint32_t pending = Register(gpio_base, REG_INTSTATUS);
  if (pending != 0u)
  {
    Register(gpio_base, REG_EOI) = pending;
  }
