...
volatile uint32_t& SG200XPWM::Register(uintptr_t base, uint32_t offset) noexcept
{
...
void SG200XPWM::ConfigurePinmux(const PinmuxConfiguration& pinmux) noexcept
{
  ...
  auto& reg = *reinterpret_cast<volatile uint32_t*>(PINMUX_BASE + pinmux.offset);
...
void SG200XPWM::WritePeriodRegisters() noexcept
{
  ...
  Register(pwm_base_, channel_base - pwm_base_ + REG_HLPERIOD) =
      period_ticks_ - high_ticks_;
  Register(pwm_base_, channel_base - pwm_base_ + REG_PERIOD) = period_ticks_;
}

void SG200XPWM::ApplyDynamicUpdate() noexcept
{
  auto& update = Register(pwm_base_, REG_PWMUPDATE);
...
ErrorCode SG200XPWM::Enable()
{
  ...
  auto& polarity = Register(pwm_base_, REG_POLARITY);
  polarity &= ~channel_mask_;

  auto& start = Register(pwm_base_, REG_PWMSTART);
  start &= ~channel_mask_;

  auto& output_enable = Register(pwm_base_, REG_PWM_OE);
...
ErrorCode SG200XPWM::Disable()
{
  ...
  Register(pwm_base_, REG_PWM_OE) &= ~channel_mask_;
  Register(pwm_base_, REG_PWMSTART) &= ~channel_mask_;
  enabled_ = false;
  return ErrorCode::OK;
}
