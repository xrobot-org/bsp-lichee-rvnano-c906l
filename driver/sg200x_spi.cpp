...
inline void ManualCsDelay() noexcept
{
  ...
  sg200x_ll_csr_delay_nops(1024u);
}

void ManualSpi2Cs(bool active) noexcept
{
  // The SD1 CS pad belongs to the always-on GPIO domain.
  sg200x_ll_gpio_output_enable(RTC_GPIO_REGS, PINMUX_SD1_D3_GPIO_MASK, true);
  sg200x_ll_gpio_output_write(RTC_GPIO_REGS, PINMUX_SD1_D3_GPIO_MASK, !active);
...
bool IsC906LChannel(uint8_t channel) noexcept
{
  return channel < DMA_CHANNEL_COUNT &&
         (SG200XDMAC::OWNED_CHANNEL_MASK & LL_BIT(channel)) != 0u;
}
}  // namespace
    ...
    : SPI(rx_buffer, tx_buffer), dma_channels_(dma_channels)
{
  ...
  if (index >= SPI_COUNT || chip_select >= SPI_CHIP_SELECT_COUNT ||
      ...
      tx_buffer.size_ == 0u ||
      (reinterpret_cast<uintptr_t>(rx_buffer.addr_) % LL_DCACHE_LINE_SIZE) != 0u ||
      (reinterpret_cast<uintptr_t>(tx_buffer.addr_) % LL_DCACHE_LINE_SIZE) != 0u ||
      (rx_buffer.size_ % LL_DCACHE_LINE_SIZE) != 0u ||
      (tx_buffer.size_ % LL_DCACHE_LINE_SIZE) != 0u)
  {
    return;
  }
...
{
  sg200x_ll_spi_slave_select_disable(regs_);
  sg200x_ll_spi_disable(regs_);
  return ErrorCode::OK;
}

ErrorCode SG200XSPI::CheckError() const
{
  if ((sg200x_ll_spi_raw_interrupt_status_get(regs_) & SPI_RISR_ERROR_MASK) != 0u)
  {
    const uint32_t clear = sg200x_ll_spi_interrupt_clear(regs_);
    (void)clear;
...
{
  return sg200x_ll_spi_wait_idle(regs_, IDLE_WAIT_ATTEMPTS) ? ErrorCode::OK
                                                       : ErrorCode::TIMEOUT;
}
...
ErrorCode SG200XSPI::SetConfig(Configuration config)
{
  ...
  (void)Disable();
  const sg200x_ll_spi_mode_t spi_mode =
      config.clock_polarity == ClockPolarity::HIGH
          ? (config.clock_phase == ClockPhase::EDGE_2 ? LL_SPI_MODE_3 : LL_SPI_MODE_2)
          : (config.clock_phase == ClockPhase::EDGE_2 ? LL_SPI_MODE_1
                                                      : LL_SPI_MODE_0);
  uint32_t ctrlr0 = sg200x_ll_spi_ctrlr0_build_motorola_8bit(spi_mode, false);
...
#endif
  sg200x_ll_spi_ctrlr0_set(regs_, ctrlr0);
  if (!sg200x_ll_spi_baud_divider_set(regs_, divider) ||
      !sg200x_ll_spi_tx_fifo_threshold_set(regs_, 0u) ||
      !sg200x_ll_spi_rx_fifo_threshold_set(regs_, 0u))
  {
    Unlock();
    return ErrorCode::NOT_SUPPORT;
  }
  sg200x_ll_spi_interrupt_mask_set(regs_, 0u);
  sg200x_ll_spi_dma_disable(regs_);
  const uint32_t clear = sg200x_ll_spi_interrupt_clear(regs_);
  (void)clear;
                                      ...
                                      sg200x_ll_dma_mode_t dma_mode)
{
  ...
  if (dma_rx.addr_ == nullptr || dma_tx.addr_ == nullptr ||
      (reinterpret_cast<uintptr_t>(dma_rx.addr_) % LL_DCACHE_LINE_SIZE) != 0u ||
      (reinterpret_cast<uintptr_t>(dma_tx.addr_) % LL_DCACHE_LINE_SIZE) != 0u ||
  ...
  circular_active_.store(dma_mode == LL_DMA_MODE_CIRCULAR, std::memory_order_release);
  if (op.type == OperationRW::OperationType::BLOCK)
  {
  ...
  (void)sg200x_ll_spi_dma_tx_level_set(regs_, 4u);
  // RX requests remain per-frame; the RX FIFO is drained by the DMA channel.
  (void)sg200x_ll_spi_dma_rx_level_set(regs_, 0u);
  sg200x_ll_spi_dma_enable(regs_, true, true);
  sg200x_ll_spi_enable(regs_);
  sg200x_ll_spi_slave_select_set(regs_, chip_select_mask_);
  const SG200XDMAC::Transfer rx_transfer{
      .memory = rx_start,
      .memory_capacity = dma_rx_capacity,
      .peripheral = sg200x_ll_spi_data_address(regs_),
      .count = frames,
      .request = static_cast<SG200XDMAC::Request>(
          sg200x_ll_dmamux_spi_request_get(controller_index_, false)),
  ...
  const SG200XDMAC::Transfer tx_transfer{
      ...
      .count = frames,
      .request = static_cast<SG200XDMAC::Request>(
          sg200x_ll_dmamux_spi_request_get(controller_index_, true)),
